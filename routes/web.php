...
$router->post('/createReservation', function (Illuminate\Http\Request $request) use ($router) {
    $reservation = new Reservation;
    $reservation->total = $request->get('total');
    $reservation->status = $request->get('status');
    $reservation->date = $request->get('date');
    $reservation->id_customer = $request->get('id_customer');

    $reservation->save();


    foreach($request["id_products"] as $product) {
        $reservation->products()->attach($product, ['price'=>'1234']);
    }
